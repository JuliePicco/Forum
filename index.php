<?php
    ...
    // on va chercher le ficher physique dans app, Autoloader.php
...
//---------REQUETE HTTP INTERCEPTEE-----------
    ...
    if(isset($_GET['ctrl'])){ // si dans l'url on trouve le mot clé "ctrl" 
        $ctrlname = $_GET['ctrl']; // alors $ctrlname prend le nom de ce qu'il ya dans l'url
    }
    ...
    //l'action par defaut est l'index, s'il ya un defaut dans l'url, s'il ne trouve pas chemin etc il renvoi qd meme une page (index) et pas une erreur
    ...
    if(isset($_GET['action']) && method_exists($ctrl, $_GET['action'])){
        ...
        $action = $_GET['action']; // alors l'action sera l'action récupérée dans l'url 
    }
    //on fait pareil avec l'id (id du topic par exemple)
    if(isset($_GET['id'])){
        $id = $_GET['id'];
    }
    else $id = null;
